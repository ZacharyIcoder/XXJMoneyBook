...
@interface BlueCralishNewAccountTableViewController () <UITextViewDelegate, UIPickerViewDataSource, UIPickerViewDelegate>
...
@implementation BlueCralishNewAccountTableViewController
...
- (void)setUpPickerView {
    ...
    if (self.pickerView == nil) {
        ...
        self.pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 0, width*4/5, height*1/4)];
        self.pickerView.layer.masksToBounds = YES;
        self.pickerView.layer.cornerRadius = 10;
        self.pickerView.center = self.tableView.center;
        self.pickerView.backgroundColor = [UIColor whiteColor];
        self.pickerView.tintColor = [UIColor hexColor:@"4C94DB"];
...
- (NSAttributedString *)pickerView:(UIPickerView *)pickerView attributedTitleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (component == 0) { // 默認第一行為支出
        ...
        // 根據收入支出類型不同分別返回不同的資料
        if ([self.incomeType isEqualToString:@"income"]) {
            title = self.incomeArray[row];
        } else {
            title = self.expenseArray[row];
        }
    }
    attString =
    [[NSAttributedString alloc] initWithString:title attributes:@{NSForegroundColorAttributeName:[UIColor hexColor:@"4C94DB"],NSFontAttributeName:[UIFont systemFontOfSize:14]}];
    return attString;
}
